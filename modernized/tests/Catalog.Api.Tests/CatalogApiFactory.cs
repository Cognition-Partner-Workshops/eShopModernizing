...
/// <summary>
/// Boots the real <c>Program</c> host but swaps the SQL Server
/// <see cref="CatalogDbContext"/> for an in-memory Sqlite connection seeded with
/// the behavioral baseline data, so the API can be exercised on CI with no
/// external database.
/// </summary>
public class CatalogApiFactory : WebApplicationFactory<Program>
{
    ...
    public CatalogApiFactory()
    {
        ...
        // minimal host is being built (before ConfigureWebHost callbacks apply),
        // so supply a non-empty placeholder via the environment. The SQL Server
        // provider it registers is replaced with Sqlite below.
