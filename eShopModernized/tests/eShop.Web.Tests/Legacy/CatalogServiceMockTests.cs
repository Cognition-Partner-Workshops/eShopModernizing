...
/// <summary>
/// The legacy <c>eShopLegacyMVC.Tests.CatalogServiceMockTests</c> MSTest suite ported to xunit.
/// It is the behavioural spec of the data the catalog UI renders in mock-data mode.
/// </summary>
public class CatalogServiceMockTests
{
    ...
    public void GetCatalogItemsPaginated_ItemsHaveBrandsPopulated()
    {
        ...
        {
            Assert.True(item.CatalogBrand is not null, $"CatalogBrand not populated for item {item.Id}");
    ...
    public void GetCatalogItemsPaginated_ItemsHaveTypesPopulated()
    {
        ...
        foreach (var item in result.Data)
        {
            Assert.True(item.CatalogType is not null, $"CatalogType not populated for item {item.Id}");
